...
args = json.load(open("config/walker_mimic.json", "r"))
...
class Walker2dEnv(MuJocoPyEnv, utils.EzPickle):
    ...
    def _calc_imitation_reward(self):
        ...
        #     print(f"num_steps={self.num_steps}, r={r}, dist={dist}, qpos={self.sim.data.qpos}, qref={self._get_qref_pose()}, qvel={self.sim.data.qvel}, qrefvel={self._get_qref_vel()}")
            # pass
        return sum([r[k] * self.w[k] for k in self.w.keys()]), sum([dist[k] * self.w[k] for k in self.w.keys()]), dist, r
    ...
    def step(self, a,  w={"goal": 0., "imitation": 1.0}):
        ...
        terminated = not (height > 0.8 and height < 2.0 and ang > -1.0 and ang < 1.0) or (imitation_dist_info['root'] > 2)
        ob = self._get_obs()
    ...
    def _get_mimic_state(self):
        qpos = self._get_qref_pose()
        qvel = self._get_qref_vel()
        return (qpos, qvel)

    def reset_model(self):
        self.num_steps = np.random.randint(0, self.cycle_time / self.dt / self.frame_skip)
        # self.set_state(
        #     self.init_qpos
